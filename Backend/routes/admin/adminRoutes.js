...
// ========================
// AUTH MIDDLEWARE (all routes protected and require admin role)
// ========================
router.use(protect);
  ...
  try {
    ...
    const bookingRes = await pool.query(`
      SELECT
        COUNT(*) as total,
        COUNT(CASE WHEN status = 'pending' THEN 1 END) as pending,
        COUNT(CASE WHEN status IN ('confirmed','upcoming') THEN 1 END) as active,
...
router.get('/dashboard/activities', async (req, res) => {
  try {
    ...
    const bookingActivities = await pool.query(`
      SELECT 'booking' as type, b.id, u.name as user_name, 'created booking #' || b.id as action, b.created_at as timestamp
...
router.get('/users', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT id, name, email, role, verified, is_active, created_at
      FROM users
...
router.put('/services/:id/reject', async (req, res) => {
  try {
    await pool.query("UPDATE services SET status = 'rejected' WHERE id = $1", [req.params.id]);
...
router.get('/payments', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT p.*, b.booking_date, u.name as customer_name
      FROM payments p
