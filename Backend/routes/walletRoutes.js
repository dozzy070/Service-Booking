...
const getWallet = async (userId) => {
  ...
  if (wallet.rows.length === 0) {
    const result = await pool.query(
      `INSERT INTO wallets (user_id, balance, points, tier, lifetime_earnings, pending_payout, total_withdrawn)
       VALUES ($1, 0, 0, 'Bronze', 0, 0, 0) RETURNING *`,
      [userId]
    );
...
// GET /api/wallet/transactions - Get all transactions with pagination
...
// GET /api/wallet/redeem-history - Get redemption history
...
// GET /api/wallet/payment-methods - Get all payment methods
...
// PUT /api/wallet/payment-methods/:id/default - Set default payment method
...
router.get('/withdrawal-methods', protect, async (req, res) => {
  try {
    const userId = req.user.id;
...
router.post('/withdrawal-methods', protect, async (req, res) => {
  try {
    const userId = req.user.id;
...
// DELETE /api/wallet/withdrawal-methods/:id - Delete withdrawal method
router.delete('/withdrawal-methods/:id', protect, async (req, res) => {
  try {
    const userId = req.user.id;
    const methodId = req.params.id;
...
// POST /api/wallet/redeem - Redeem reward
