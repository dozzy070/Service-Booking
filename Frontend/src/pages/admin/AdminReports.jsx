...
const Analytics = () => {
  ...
  // Analytics Data – all defaults are 0 or empty (no mock)
  ...
  // All API calls (no loading state)
  const fetchOverview = async () => {
    try {
      ...
      showToast('Failed to load overview', 'danger');
    }
  };
  ...
  // Review actions (real API calls)
  const handleApproveReview = async (reviewId) => {
    try {
      await api.put(`/admin/analytics/reviews/${reviewId}/approve`);
      await fetchReviews();
  ...
  const handleFlagReview = async () => {
    ...
    try {
      await api.put(`/admin/analytics/reviews/${selectedReview.id}/flag`);
      await fetchReviews();
  ...
  const handleDeleteReview = async () => {
    if (!selectedReview) return;
    try {
      await api.delete(`/admin/analytics/reviews/${selectedReview.id}`);
      await fetchReviews();
  ...
  const handleSubmitResponse = async () => {
    ...
    try {
      await api.post(`/admin/analytics/reviews/${selectedReview.id}/respond`, { response: responseText });
      await fetchReviews();
  ...
  const handleBulkAction = async (action) => {
    try {
      await api.post('/admin/analytics/reviews/bulk', { reviewIds: selectedReviews, action });
      await fetchReviews();
      setSelectedReviews([]);
  ...
  // Local filtering (client-side fallback)
  useEffect(() => {
  ...
  // Initial load and auto-refresh (no spinners)
  useEffect(() => {
    fetchAllData();
  ...
  // Export functions (remain the same, but use real data)
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(20);
      ...
      <Row className="mb-4">
        ...
        <Col xs="auto" className="d-flex gap-2">
          ...
          <Dropdown>
            ...
            <Dropdown.Menu>
              <Dropdown.Item onClick={exportToPDF}><FaFilePdf className="me-2" /> PDF</Dropdown.Item>
              <Dropdown.Item onClick={exportToExcel}><FaFileExcel className="me-2" /> Excel</Dropdown.Item>
              <Dropdown.Item onClick={exportToCSV}><FaFileAlt className="me-2" /> CSV</Dropdown.Item>
            </Dropdown.Menu>
          </Dropdown>
          ...
          <Row className="g-4 mb-4">
            ...
            <Col lg={4}>
              <Card className="border-0 shadow-sm h-100">
                ...
                <Card.Body>{distribution.usersByRole.map((item, idx) => (<div key={idx} className="mb-3"><div className="d-flex justify-content-between"><span>{item.role}</span><span className="fw-bold">{item.count}</span></div><ProgressBar now={item.percentage} variant={item.role === 'Customer' ? 'primary' : 'success'} style={{ height: '8px' }} /><small className="text-muted">{item.percentage}%</small></div>))}</Card.Body>
              </Card>
            </Col>
          </Row>
          ...
          <Row className="g-4">
            <Col lg={6}>
              <Card className="border-0 shadow-sm">
                ...
                <Card.Body>{distribution.servicesByCategory.map((item, idx) => (<div key={idx} className="mb-3"><div className="d-flex justify-content-between"><span>{item.category}</span><span className="fw-bold">{item.count}</span></div><ProgressBar now={item.percentage} variant="info" style={{ height: '8px' }} /><small className="text-muted">{item.percentage}%</small></div>))}</Card.Body>
              </Card>
            </Col>
            <Col lg={6}>
              <Card className="border-0 shadow-sm">
                <Card.Header className="bg-white border-0 pt-4"><h5 className="mb-0">Revenue by Source</h5></Card.Header>
                <Card.Body>{distribution.revenueBySource.map((item, idx) => (<div key={idx} className="mb-3"><div className="d-flex justify-content-between"><span>{item.source}</span><span className="fw-bold">{formatCurrency(item.amount)}</span></div><ProgressBar now={item.percentage} variant="success" style={{ height: '8px' }} /><small className="text-muted">{item.percentage}%</small></div>))}</Card.Body>
              </Card>
            </Col>
          </Row>
        </>
        ...
        <>
          ...
          <Card className="border-0 shadow-sm mb-4">
            <Card.Body>
              ...
              {selectedReviews.length > 0 && (<div className="d-flex gap-2 mt-3 pt-3 border-top"><Button size="sm" variant="success" onClick={() => handleBulkAction('approved')}><FaCheckCircle className="me-2" /> Approve ({selectedReviews.length})</Button><Button size="sm" variant="danger" onClick={() => handleBulkAction('flagged')}><FaFlag className="me-2" /> Flag</Button><Button size="sm" variant="outline-danger" onClick={() => handleBulkAction('deleted')}><FaTrash className="me-2" /> Delete</Button></div>)}
            </Card.Body>
          </Card>
                  ...
                  <tbody>
                    {currentReviews.map(review => (
                      <tr key={review.id} className={selectedReviews.includes(review.id) ? 'table-active' : ''}>
                        ...
                        <td><div className="text-center"><div className="fw-semibold">{review.helpful}</div><small className="text-muted">{review.notHelpful}</small></div></td>
                        <td><small><FaCalendarAlt className="me-1" /> {new Date(review.createdAt).toLocaleDateString()}</small></td>
                        <td><div className="d-flex gap-2"><Button size="sm" variant="outline-primary" onClick={() => { setSelectedReview(review); setShowReviewModal(true); }}><FaEye /></Button>{review.status === 'pending' && <Button size="sm" variant="outline-success" onClick={() => handleApproveReview(review.id)}><FaCheckCircle /></Button>}{review.status !== 'flagged' && <Button size="sm" variant="outline-danger" onClick={() => { setSelectedReview(review); setShowFlagModal(true); }}><FaFlag /></Button>}{!review.response && <Button size="sm" variant="outline-info" onClick={() => { setSelectedReview(review); setShowResponseModal(true); }}><FaReply /></Button>}<Dropdown><Dropdown.Toggle size="sm" variant="outline-secondary"><FaEllipsisV /></Dropdown.Toggle><Dropdown.Menu><Dropdown.Item onClick={() => { setSelectedReview(review); setShowDeleteModal(true); }}><FaTrash className="me-2 text-danger" /> Delete</Dropdown.Item></Dropdown.Menu></Dropdown></div></td>
                      </tr>
                    ))}
                  </tbody>
                </Table>
              </div>
              {filteredReviews.length === 0 && (<div className="text-center py-5"><FaStar size={48} className="text-muted mb-3" /><h5>No reviews found</h5></div>)}
              {filteredReviews.length > 0 && (<div className="d-flex justify-content-between align-items-center p-3 border-top"><div>Showing {indexOfFirstItem + 1} to {Math.min(indexOfLastItem, filteredReviews.length)} of {filteredReviews.length}</div><Pagination><Pagination.Prev onClick={() => setCurrentPage(p => Math.max(p-1,1))} disabled={currentPage===1} />{[...Array(Math.min(5,totalPages)).keys()].map(num => { let pageNum; if(totalPages<=5) pageNum=num+1; else if(currentPage<=3) pageNum=num+1; else if(currentPage>=totalPages-2) pageNum=totalPages-4+num; else pageNum=currentPage-2+num; return <Pagination.Item key={pageNum} active={pageNum===currentPage} onClick={()=>setCurrentPage(pageNum)}>{pageNum}</Pagination.Item>;})}<Pagination.Next onClick={()=>setCurrentPage(p=>Math.min(p+1,totalPages))} disabled={currentPage===totalPages} /></Pagination></div>)}
            </Card.Body>
          </Card>
        </>
      )}
